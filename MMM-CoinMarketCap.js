...
Module.register("MMM-CoinMarketCap", {
  ...
  getCell: function (colType, currency) {
    ...
    switch (colType) {
      ...
      case "change7d":
        ...
        if (
          currency.percentChangeColored &&
          data.quote[conversion].percent_change_7d >= 0
        ) {
          cell.classList.add("positive");
        }
        if (
        ...
        cell.innerHTML = Number(
          data.quote[conversion].percent_change_7d / 100
        ).toLocaleString(config.language, {
          style: "percent",
          currency: self.config.conversion,
          maximumFractionDigits: 2,
      ...
      case "logo":
        ...
        var LocalLogoFileName =
          self.httpLogoFolder +
          data.symbol.toLowerCase() +
          "-" +
          currency.logoSizePX +
          ".png";
        var LocalLogoFileNameBW =
          self.httpLogoFolderBW +
          data.symbol.toLowerCase() +
          "-" +
          currency.logoSizePX +
          ".png";
        ...
        var logo = new Image();
        if (currency.logoColored && LocalLogoExists) {
      ...
      case "graph":
        ...
        graphURL = self.replaceAll(
          graphURL,
          "{range}",
          self.config.graphRange.toString()
        );
        graphURL = self.replaceAll(
          graphURL,
          "{noCache}",
        ...
        if (self.config.graphColored && self.config.graphRange === 1) {
          graph.classList.add(
            data.quote[conversion].percent_change_24h >= 0 ? "up" : "down"
          );
        } else if (self.config.graphColored && self.config.graphRange === 7) {
          graph.classList.add(
            data.quote[conversion].percent_change_7d >= 0 ? "up" : "down"
          );
        } else {
  ...
  conformNumber: function (number, significantDigits, decimalPlaces) {
    var self = this;
    if (
      !axis.isNumber(significantDigits) ||
      isNaN(significantDigits) ||
      significantDigits < 1
    ) {
      significantDigits = 0;
    }
    if (
      !axis.isNumber(decimalPlaces) ||
      isNaN(decimalPlaces) ||
      decimalPlaces < 0
    ) {
      decimalPlaces = -1;
    ...
    if (significantDigits === 0) {
      if (decimalPlaces === -1) {
        ...
        result = self.roundNumber(number, decimalPlaces).toFixed(decimalPlaces);
      }
    } else {
      var integerPartSize =
        Math.floor(number) === 0 ? 0 : Math.floor(number).toString().length;
      if (decimalPlaces === -1) {
        if (integerPartSize === 0) {
          // Handle the special case: 0 < number < 1
          var parts = number.toExponential(significantDigits).split("e");
          result = Number(
            self.roundNumber(parts[0], significantDigits - 1) + "e" + parts[1]
          );
        } else {
          result = self.roundNumber(
            number,
            significantDigits - integerPartSize
          );
        }
      } else {
        result = self.conformNumber(
          self.conformNumber(number, significantDigits, -1),
          0,
          decimalPlaces
        );
      }
  ...
  roundNumber: function (number, precision) {
    if (precision >= 0) {
      ...
      return Number(
        Math.round(number + "e-" + Math.abs(precision)) +
          "e" +
          Math.abs(precision)
      );
    }
  ...
  getLogoURL: function (size, id) {
    var self = this;
    if (axis.isString(size)) {
      size = self.logoSizeToPX[size];
    }
