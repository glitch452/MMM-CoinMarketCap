...
module.exports = NodeHelper.create({
  ...
  start: function () {
    ...
    try {
      self.expressApp.use(
        "/" + self.name + "/logos",
        express.static(self.path + "/logos")
      );
  ...
  socketNotificationReceived: function (notification, payload) {
    ...
    if (payload.developerMode) {
      console.log(
        self.name + ': Socket Notification Received: "' + notification + '".'
      );
    }
    if (notification === "GET_LISTINGS") {
      ...
      self.sendSocketNotification("LOG", {
        original: payload,
        message: "INIT received from: " + payload.instanceID + ".",
        messageType: "dev",
      });
      self.sendSocketNotification("LOG", {
        original: payload,
        message: "node_helper.js loaded successfully.",
        messageType: "dev",
      });
    }
  },
  ...
  downloadFile: function (payload) {
    var self = this;
    request(
      { url: payload.url, encoding: "binary", method: "GET" },
            ...
            function (err) {
              if (err) {
                self.sendSocketNotification("LOG", {
                  original: payload,
                  translate: true,
                  message: "LOGO_SAVE_FAILURE",
                  translateVars: {
                    sourceFilename: payload.url,
                    destinationFilename: payload.saveToFileName,
                  },
                });
              } else {
                self.sendSocketNotification("LOG", {
